...
iNavi Maps MCP Server 사용 중 발생할 수 있는 문제들과 해결 방법을 정리했습니다.
...
### 증상
Claude Desktop이나 다른 MCP Host에서 iNavi Maps MCP 서버를 인식하지 못합니다.
...
2. **네트워크 확인**
   ...
   ```bash
   curl https://dev-imaps.inavi.com/maps/v3.0
   ```
...
1. **API 키 확인**
   ...
   <script src="https://dev-maps.inavi.com/maps/v3.0/appkeys/{appKey}/maps?callback=initMap"></script>

   <!-- {appKey}를 실제 API 키로 교체 -->
   <script src="https://dev-maps.inavi.com/maps/v3.0/appkeys/your_api_key_here/maps?callback=initMap"></script>
   ```
...
1. **GitHub Issues**: [문제 보고하기](https://github.com/inavi-systems/inavi-mcp/issues)
2. **로그 첨부**: MCP Host의 로그 파일을 첨부해주시면 더 빠르게 도움을 드릴 수 있습니다
3. **환경 정보**: OS, Node.js 버전, MCP Host 종류 및 버전을 함께 알려주세요
